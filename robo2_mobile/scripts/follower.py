...
class mymobibot_follower():
    ...
    def __init__(self,rate):
        ...
        self.ang_vel = rospy.Publisher('/mymobibot/ang_vel', Float64, queue_size=1)
        self.ang_err = rospy.Publisher('/mymobibot/ang_err', Float64, queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if self.phase == 'initial':
                ...
                if sonar_front < 0.1+self.desired_distance:
                    self.phase = 'wall_following'
                    prev_left_err = 0
                    prev_front_err = 0

            if self.phase == 'wall_following':
                """
                > X = X1 + X2 = 1+9 = 10
                > X mod 2 = 0
                > Clockwise rotation
                ...
                left_dt_err = (left_err - prev_left_err) / dt
                prev_left_err = left_err
                
                ang_err = (sonar_left+0.1)/(sonar_front_left+0.2/sqrt(2)) - sqrt(2)/2

                
                if sonar_front < self.front_desired_distance: # steering
                    front_err = sonar_front - self.front_desired_distance
                    front_derivative_error = (front_err - prev_front_err) / dt
                    prev_front_err = front_err
                    self.velocity.angular.z = -(kp*left_err + kd*left_dt_err) \
                                                 + kp_ang*ang_err \
            ...
            # self.ang_vel.publish(self.velocity.angular.z)
            # self.ang_err.publish(ang_error)
